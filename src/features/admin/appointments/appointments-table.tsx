...
export function AppointmentsTable({ appointments, isLoading, onAction, pendingId, errorMessage }: AppointmentTableProps): JSX.Element {
  if (isLoading) {
    return (
  ...
  if (!appointments.length) {
    return (
      <div className="rounded-lg border border-dashed border-muted p-6 text-sm text-muted-foreground">
        Nenhum agendamento encontrado com os critérios atuais.
      </div>
    );
  }
              ...
              return (
                <tr key={appointment.id} className="hover:bg-muted/30">
                  <td className="px-4 py-3 text-sm text-foreground">
                    {appointment.customerName ?? 'Sem nome informado'}
                  </td>
